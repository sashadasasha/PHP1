...
if (e.target.classList[0] === "delete") {
 ...
 if (e.target.classList[0] === "imgItem" || e.target.classList[0] === "nameItem" ) {
  ...
  const modalWindow = document.querySelector('.modalWindow');
    ...
    modalWindow.insertAdjacentHTML('beforeend', `<div id="${idImg}mod"><img src="/img/${result['imgAddr']}" alt="" height="200"      width="200"><div class="info">
...
if (e.target.classList[0] === "orderItem") {
  e.preventDefault();
  console.log(e.target.value);
  let id = e.target.id;
  let data = {id}; 

  let result = await responseAPI("orderItem", data);
  ...
  const modalWindow = document.querySelector('.modalWindow');
    ...
    modalWindow.insertAdjacentElement('beforeend', container);
    result.forEach(element => {
      console.log(element);
      container.insertAdjacentHTML('beforeend', `<div><div >
      <p href="/api/deleteOrderItem/"><p>${element['name']}</p><p>Цена - ${element['price']}</p></div>`);

    });
    
    container.insertAdjacentHTML('beforeend', `<div>Заказ от ${result[0]['login']}.
                                                  Контактные данные - ${result[0]['tel']}
                                                  Адрес доставки - ${result[0]['address']}
                                                </div>`);
